...
def load_model():
    ...
    """
    model_hf = pipeline(config["task"], model=config["model"], device=-1) # импорт модели с Hugging Face

    def model(text: str) -> SentimentPrediction:
        pred = model_hf(text) # в нашем случае model_hf содержит перпроцессинг текст и токенизацию, инференс модели и постпроцессинг предсказаний
